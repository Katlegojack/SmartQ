...
from queues.resilient_day import (
    ...
    SCENARIO_RECEPTIONIST_PASSWORD,
...
class Command(BaseCommand):
    help = "Seed the Day 62 resilient Smart Q training day with deterministic no-shows."
    ...
    def handle(self, *args, **options):
        if getattr(settings, "IS_PRODUCTION", False):
            raise CommandError("seed_resilient_day is disabled in production.")
        ...
        if customer_count < 10:
            raise CommandError("Use at least 10 customers.")
        if no_show_count < 0 or no_show_count >= customer_count:
            raise CommandError("--no-shows must be between 0 and customers-1.")

        try:
            no_show_positions(customer_count, target_date, no_show_count)
        except ValueError as exc:
        ...
        if User.objects.filter(username__startswith=prefix).exists():
            if not options["reset"]:
                raise CommandError(
                    f"Training customers already exist for {target_date}. Use --reset to rebuild."
                )
        ...
        with transaction.atomic():
            ...
            staff = self._ensure_counter_staff(branch)
            self._ensure_counters(branch, staff)
            self._ensure_observers(branch)
        ...
        planned_absences = no_show_positions(customer_count, target_date, no_show_count)

        self.stdout.write(self.style.SUCCESS("Day 62 resilient training scenario is ready."))
        self.stdout.write(f"Date: {target_date}")
        self.stdout.write(f"Branch: {SCENARIO_BRANCH_NAME} ({SCENARIO_BRANCH_CODE})")
        self.stdout.write(
            f"Seeded customers: {bookings.count()} = {general_count} General + {priority_count} Priority"
        )
        self.stdout.write(f"Planned no-shows: {len(planned_absences)} after a 10-minute grace period")
        self.stdout.write("Counters: 2 General + 1 Priority (Priority may overflow to General)")
        self.stdout.write("Operating window: 09:00-18:00; appointments: 09:00-17:45")
        self.stdout.write("Live customer/reception walk-ins are EXTRA and join the same queue.")
        self.stdout.write("")
        self.stdout.write("Service targets and reproducible actual-duration ranges:")
        for service_code in SERVICE_SEQUENCE:
            service_bookings = [
                booking
                for booking in bookings
                if booking.service.service_code == service_code
            ]
            values = [
        ...
        if SCENARIO_MANAGER_PASSWORD:
            self.stdout.write(f"Manager observer enabled: {SCENARIO_MANAGER_USERNAME}")
        else:
            self.stdout.write("Manager observer login disabled until SMARTQ_TRAINING_MANAGER_PASSWORD is set and reseeded.")
        if SCENARIO_RECEPTIONIST_PASSWORD:
            self.stdout.write(f"Reception observer enabled: {SCENARIO_RECEPTIONIST_USERNAME}")
        else:
            self.stdout.write("Reception observer login disabled until SMARTQ_TRAINING_RECEPTION_PASSWORD is set and reseeded.")

        self.stdout.write(
            f"Next: python manage.py verify_resilient_day --date {target_date} "
            f"--customers {customer_count} --no-shows {no_show_count}"
        )
    ...
    def _ensure_branch(self):
        branch, _ = Branch.objects.update_or_create(
            ...
            defaults={
                ...
                "city": "Pretoria",
                "opening_time": OPERATING_START,
                "closing_time": OPERATING_END,
                "is_active": True,
            },
        )
        return branch
    ...
    def _ensure_counter_staff(self, branch):
        users = {}
        for username, first_name, last_name, _queue_type, _counter_number in STAFF_SPECS:
    ...
    def _ensure_observers(self, branch):
        ...
        self._ensure_user_profile(
            username=SCENARIO_RECEPTIONIST_USERNAME,
            first_name="Training",
            last_name="Reception",
            ...
            gender=Profile.OTHER,
            disability_status=False,
            usable_password=SCENARIO_RECEPTIONIST_PASSWORD,
        )
    ...
    def _ensure_counters(self, branch, staff):
        for username, _first, _last, queue_type, counter_number in STAFF_SPECS:
            ...
            counter, _ = Counter.objects.update_or_create(
                branch=branch,
                counter_number=counter_number,
                defaults={"queue_type": queue_type, "status": Counter.CLOSED},
            )
